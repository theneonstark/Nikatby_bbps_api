...
class BbpsController extends Controller
{
    public function services(){
        ...
        ->toArray(); 
        // dd($services);
        //Only category is sending to frontend billpay
        // dd($services);
        // $services = BbpsServices::where('blr_category_name', 'Broadband Postpaid');
        // dd($services);
    ...
    public function getAllOperator($category) {
        // dd($category);
        $decodedCategory = urldecode($category); // Correct usage
        // dd($decodedCategory);
        ...
        // dd($services->blr_id);
        // UtilityOperator::where('category', $category)->pluck('name');
    
        return Inertia::render('CategoryDetail', [  // Make sure file name matches
    ...
    // }
    // protected $accessCode = 'AVCU98BI86TU91GDGO'; for uat bbps
    protected $accessCode = 'AVQU51SS09TR19KLWN'; //for live bbps
    // , $biller, $type
    public function billerId(Request $request, $biller)
    {
        // dd($biller); //'Abhushan Residency C Wing'
        $billerNameId = BbpsServices::where('blr_name', $biller)->first();
        // dd($billerId->blr_id);
        try{
            $requestId = \MyHelper::generateRequestId();
            // 98pjO63P1123k5pq89lll56p21m2345n78v
        //   dd('Coming');
            // $key = '48ECC74C29EB85471E7511C54C0F310A'; for uat bbps
            ...
            dd($billerId);
            // $url = "https://stgapi.billavenue.com/billpay/extMdmCntrl/mdmRequestNew/xml?accessCode=AVCU98BI86TU91GDGO&requestId=98pjO63P1123k5pq89lll56p21m2345n78v&ver=1.0&instituteId=NT05"; //for uat bbps
            $url = "https://api.billavenue.com/billpay/extMdmCntrl/mdmRequestNew/xml?accessCode=AVQU51SS09TR19KLWN&requestId=".$requestId."&ver=1.0&instituteId=RP16"; // for live bbps
            // dd($url);
            $header = array(
